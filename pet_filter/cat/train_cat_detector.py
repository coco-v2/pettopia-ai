...
#훈련
cat_detector_model.compile(optimizer=keras.optimizers.Adam(), loss='mse')

cat_detector_model.fit(x_train, y_train, epochs=50, batch_size=32, shuffle=True,
                       validation_data=(x_test, y_test), verbose=1,
                        callbacks=[
                            TensorBoard(log_dir='log/%s' % (start_time)),
                            ModelCheckpoint('model/head_detector/%5.h' % (start_time), monitor='val_loss', verbose=1, save_best_only=True, mode='auto'),
